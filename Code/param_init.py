...
def initialize_param(N, distribute=None, specs=None, normalize=False, terzaghi=False):
    """
    ...
    # Prepare a normalization info dictionary if normalization is used.
    if normalize:
        norm_info = {}
        norm_info['t'] = specs['t']
    ...
    for param in [key for key in specs.keys() if key != 't']:
        spec = specs[param]
        mean = spec['mean']
        std = spec['std']
        ...
        if terzaghi and param == 'H':
            # Terzaghi's model requires a depth parameter.
            z_coll = tensor* torch.rand(N, 1) # Random depth
            if normalize:
                # normalize the depth to between 0 and 1
                z_coll/= tensor
            z_coll.requires_grad_(True) 
            params['z_coll'] = z_coll
    ...
    params['t0'] = t0
    if terzaghi:
        z0 = torch.zeros_like(t_coll).clone().detach().requires_grad_(True)
        z1 = torch.ones_like(t_coll).clone().detach().requires_grad_(True)
        params['z0'] = z0
        params['z1'] = z1
